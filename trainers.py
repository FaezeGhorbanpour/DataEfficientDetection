...
class WeightedTrainer(Trainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False, **kwargs):
        ...
        # compute custom loss
        weighted_loss_fct = torch.nn.CrossEntropyLoss(weight=self.class_weights)
        loss = weighted_loss_fct(logits, labels)
        return (loss, outputs) if return_outputs else loss


class RetrievalWeightedTrainer(Trainer):
    def __init__(self, *args, retrieval_loss_weight, **kwargs):
        super().__init__(*args, **kwargs)
...
class CurriculumLearningTrainer(Trainer):
    """
    Trainer with curriculum learning that gradually adds retrieved samples based on score and dist.
    """

    def __init__(self, *args, total_epochs=10, schedule_type="linear", schedule_order='ascending', **kwargs):
        super().__init__(*args, **kwargs)
        self.total_epochs = total_epochs
        self.schedule_type = schedule_type
        self.schedule_order = schedule_order
        self.current_epoch = None  # Track the epoch

        # Store the full dataset but start training with only the target language data
        self.full_train_dataset = self.train_dataset
        self.target_dataset = self._initialize_target_only_dataset()
        self.retrieved_dataset = self._initialize_retrieved_dataset()

        # Start training with only the target dataset
        self.train_dataset = self.target_dataset
        self.epoch_train_size = dict()

    def _initialize_target_only_dataset(self):
        """
        Extracts only the target language dataset from the full dataset.
        """
        return self.full_train_dataset.filter(lambda example: example["source"] == 0)

    def _initialize_retrieved_dataset(self):
        """
        Extracts only the retrieved samples from the full dataset and sorts them by score (descending)
        and dist (ascending).
        """
        retrieved = self.full_train_dataset.filter(lambda example: example["source"] == 1)

        # Sort retrieved instances: prioritize high score, then lowest dist
        retrieved = retrieved.sort("score", reverse=False if self.schedule_order == "ascending" else True)

        return retrieved

    def _get_retrieval_ratio(self):
        """
        Computes the percentage of retrieved samples to include at the current epoch.
        ...
        if self.schedule_type == "linear":
            ...
            return 1 - np.exp(-epoch / (total_epochs / 3))
    ...
    def get_train_dataloader(self):
        """
        Updates train_dataset before each epoch and returns a new dataloader.
        """
    ...
    def training_step(self, model, inputs, *args, **kwargs):
        """
        Hook into the training step to track epochs and modify dataset per epoch.
        """
    ...
    def compute_loss(self, model, inputs, return_outputs=False, **kwargs):
        ...
        logits = outputs.get("logits")
        # compute custom loss
        weighted_loss_fct = torch.nn.CrossEntropyLoss()
        loss = weighted_loss_fct(logits, labels)
        return (loss, outputs) if return_outputs else loss

    def compute_initial_max_steps(self, num_epoch, batch_size, grad_accumulation):
        """
        Computes the initial max_steps for Hugging Face Trainer based on curriculum learning schedule.

        Args:
            schedule_type (str): "linear", "exponential", or "stepwise".
            m (int): Initial size of the training set.
            n (int): Size of the retrieved dataset.
            epoch (int): Total number of training epochs.
            batch_size (int): Batch size used for training.
            grad_accumulation (int): Gradient accumulation steps.

        Returns:
            int: Computed max_steps value.
        """
        ...
        # Compute total training samples
        total_train_samples = avg_train_size * num_epoch

        # Compute max_steps
        max_steps = math.ceil(total_train_samples / (batch_size * grad_accumulation))
